...
Configuration DemoLabEnvironment {
    ...
    Node $AllNodes.NodeName {
        ...
        if ($Node.Environment.Host -and $Node.Environment.Host.Share) {
            $hostName = $Node.Environment.Host.Name
            $sharePath = "\\$hostName\$($Node.Environment.Host.Share.Name)"
            $shareCredential = New-Object -TypeName PSCredential -ArgumentList "$hostName\$($Node.Environment.Host.Share.UserName)",(ConvertTo-SecureString -String $($Node.Environment.Host.Share.Password) -AsPlainText -Force)
        }
